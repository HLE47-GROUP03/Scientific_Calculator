...
class SciCalc():
    ...
    def secOpSelect(self):
        ...
        elif self.secOperation=='nPower':                           # Αν δεν έχει αποθηκευτεί η μεταβλητή της βάσης, χρήση του αριθμού που δόθηκε σαν βάση
            if self.haveOperant==False:
                self.base=self.floatOrInt()                          # Η τιμή της οθόνης αποθηκεύεται στη μεταβλητή βαθμού ρίζας
                self.haveOperant=True
                self.result=True
            else:
                self.exponent=self.floatOrInt()
                self.secTotal=self.base**self.exponent
                display.delete(0, 'end')
                display.insert(0,self.secTotal)                     # Εμφάνιση στην οθόνη του αποτελέσματος
                self.haveOperant=False                              # Εφόσον έγινε η πράξη, η μεταβλητή ύπαρξης πρώτου τελεστέου γίνεται πάλι ψευδής
                self.result=True
