...
class SciCalc():
    ...
    def secOpSelect(self):
        if self.secOperation=='nRoot':
            if self.haveDegree==False:                              # Αν δεν έχει αποθηκευτεί η μεταβλητή του βαθμού της ρίζας, χρήση του αριθμού που δόθηκε σαν βαθμός
                self.degree=self.floatOrInt()
                self.haveDegree=True
            else:
                self.radicand=self.floatOrInt()                     # Αν υπάρχει ήδη βαθμός, χρήση του αριθμού ως υπόρριζο 
                self.secTotal=self.radicand**(1/self.degree)
                display.delete(0, 'end')
                display.insert(0,self.secTotal)
                self.result=True
