...
class SciCalc():
    ...
    def opSelect(self):                                             # για τις βασικές πράξεις (+, -, *, /)  και το '='
        if self.operation=='addition':
            ...
            if self.result==False:                # έλεγχος αν υπάρχει ήδη ένας τελεστέος, ΄ώστε να μην αφαιρεθεί ο πρώτος τελεστέος από το '0' που είναι η αρχικοποιημένη τιμή της μεταβλητής self.total
                self.total = self.floatOrInt()
            else:
                self.total -= self.floatOrInt()
