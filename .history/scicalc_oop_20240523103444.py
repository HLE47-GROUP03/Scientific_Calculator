        ...
        if self.secOperation=='nRoot':                              # # Υπολογισμός n-οστής ρίζας του Χ
            ...
            try:
                ...
                if is_deg:                                          # Αν ο επιλογέας υπολογισμού γωνιών είναι σε μοίρες
                    self.secTotal=math.degrees(self.angle)          # Μετατροπή απο ακτίνια σε μοίρες (η math.asin επιστρέφει αποτέλεσμα σε ακτίνια)
                    self.printNumber(self.secTotal)                         # Εμφάνιση στην οθόνη του αποτελέσματος
                    self.result=False
                    self.secOperation=None                                  # Μηδενισμός της μεταβλητής επιλογής δευτερεύουσας πράξης
            except:
                self.secTotal='ERROR'                               # Εξαίρεση σφάλματος για την περίπτωση που το δεδομένο δεν είναι ανάμεσα στο -1 και το 1 (προϋποθεση της math.asin()) 
...
functions_1=['', '', '',
             ...
             calc.nPower, calc.squared, calc.sin, calc.cos, '',
             '', '', '', '', '',
             '', '', '', '', '',
